...
export function createSummonerRig() {
// ─── Materials ─────────────────────────────────────────────────────────────────
...
// ════════════════════════════════════════════════════════
  ...
  function update(dt, moving) {
    ...
    model.position.x = 0;
    model.position.z = 0;
    model.position.y = alignOffset + Math.sin(t * 0.9) * (moving ? 0.06 : 0.14) + Math.sin(t * 1.8) * 0.02;

    // Rotação corporal lenta — olha ligeiramente para os lados
    if (moving) model.rotation.y = Math.sin(t * 0.25) * 0.12; else model.rotation.y = Math.sin(t * 0.25) * 0.18;

    // Tilt lateral mínimo — equilíbrio flutuante
    model.rotation.z = Math.sin(t * 0.6) * 0.025;

    // Inclinação frontal quase nula
    model.rotation.x = 0.04 + Math.sin(t * 0.35) * 0.015;
