...
// ─── Lighting ──────────────────────────────────────────────────────────────────
const ambientLight = new THREE.AmbientLight(0x331108, 2.5);
...
const rimLight = new THREE.DirectionalLight(0x220066, 1.5);
rimLight.position.set(-8, 6, -5);
scene.add(rimLight);

const fillLight = new THREE.DirectionalLight(0x221100, 1.0);
fillLight.position.set(0, -3, 6);
scene.add(fillLight);

const reaperGlow = new THREE.PointLight(0xff4400, 4.5, 10);
reaperGlow.position.set(0, 2.5, 1.5);
scene.add(reaperGlow);

const eyeGlow = new THREE.PointLight(0xffaa00, 3.5, 8);
...
function animate() {
  ...
  model.position.y = 0.5 + Math.abs(Math.sin(walkPhase)) * 0.16;
  ...
  reaperGlow.intensity = 3.0 + Math.sin(_t * 1.14) * 1.5;
  eyeGlow.intensity    = 2.5 + Math.sin(_t * 1.57) * 1.0;
